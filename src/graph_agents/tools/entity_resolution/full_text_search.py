...
def _get_python_type_from_spanner_type(spanner_type: str):
    ...
    if spanner_type.startswith(array_prefix) and spanner_type.endswith(array_suffix):
...
def _get_alias(name: str, aliases: Optional[Dict[str, str]] = None) -> Optional[str]:
    if aliases is None:
        return name
...
def _build_full_text_search_function_description(
    ...
    function_name = (
        f"resolve_{table_alias}_{canonicalized_fields_name}_by_{input_fields_name}"
    )
    example_doc = ""
...
def _build_search_criterias(
    ...
    for col in tokenlist_cols:
        if col.expr is None:
            logger.warning(f"No expr found for tokenlist column definition: {col.name}")
            return None
        match = re.match(
        ...
        search_criterias.append(
            SearchCriteria(
                ...
                tokenized_column=ColumnWithAlias(column=tokenized_column, alias=alias),
...
def _build_full_text_search_function(
    ...
    if include_all_index_fields:
        for col in index.columns:
            ...
            if alias is None:
                continue
            canonical_reference_fields.append(ColumnWithAlias(column=col, alias=alias))
    ...
    if CanonicalReference is None:
        logger.warning(f"Unable to build {table_alias}CanonicalReference data model")
        return None
    logger.debug(
    ...
    class ReferenceMapping(BaseModel):
        ...
        reference_in_user_query: Any = Field(
            description=f"A reference of {table_alias} from user input query"
        )
        canonical_references: List[Any] = Field(
            description=(
