...
class Dataset(object):
    ...
    def __init__(self, path, delimiter: str = ","):
        ...
        self.parameter_providers: Dict[str, Callable[[], Tuple[Any, Any]]] = {}
    ...
    def load(
        ...
        self._load_nodes(db, max_batch_size)
        logger.info("finished loading nodes")

        logger.info("loading edges")
        self._load_edges(db, max_batch_size)
    ...
    def load_evalution_templates(self) -> Dict[str, List[Dict[str, Any]]]:
    ...
    async def get_agent_answer(self, agent: BaseAgent, question: str) -> str:
                ...
                for question in questions:
                    for _, param_name, _, _ in string.Formatter().parse(question):
                        ...
                        if param_name not in self.parameter_providers:
                            raise ValueError(
                                f"Missing param provider for `{param_name}`"
                            )
